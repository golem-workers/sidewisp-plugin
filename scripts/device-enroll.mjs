...
try {
  ...
  if (!['begin','poll','watch-hermes'].includes(action)) throw new Error('expected_begin_poll_or_watch_hermes');
  let result;
  ...
  } else result = action === 'begin' ? await client.begin({ id, runtime }) : await client.poll();
