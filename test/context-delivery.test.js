...
test('context samples at 30s start cadence without full diagnostics; stop prevents rescheduling',async()=>{
 ...
 const delivery=createContextUsageDelivery({now:()=>clock,collect:async()=>{calls++;return [{key:'context.used',value:123},{key:'context.capacity',value:456},{key:'context.measured_at_ms',value:1800000000000-86400000}];},
 ...
 clock+=delay;callback();await flush();assert.equal(bodies[1].observedAtMs-bodies[0].observedAtMs,30000);assert.equal(bodies[1].measuredAtMs,bodies[0].measuredAtMs);assert.equal(bodies[1].used,bodies[0].used);
